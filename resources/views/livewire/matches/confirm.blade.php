...
<div class="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
    <div class="rounded-lg bg-white p-8 shadow-lg">
        <p class="text-xs font-black uppercase tracking-[.25em] text-[#d6a31d]">{{ $match->format }} match</p>
        <h1 class="mt-2 text-3xl font-black text-[#071a80]">Confirm result</h1>
        <p class="mt-2 text-blue-950/60">Played {{ $match->played_at->format('M j, Y') }}. Winner: Side {{ $match->winner_side }}.</p>

        <div class="mt-8 grid gap-5 md:grid-cols-2">
            @foreach (['A', 'B'] as $side)
                <section class="rounded-md border border-blue-950/10 p-5">
                    <h2 class="font-black text-[#071a80]">Side {{ $side }}</h2>
                    <div class="mt-4 space-y-3">
                        @foreach ($match->players->where('side', $side)->sortBy('position') as $player)
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-xs font-black uppercase {{ $player->confirmed_at ? 'text-green-600' : 'text-blue-950/40' }}">{{ $player->confirmed_at ? 'Confirmed' : 'Waiting' }}</span>
                            </div>
                        @endforeach
                    </div>
                </section>
        ...
        <div class="mt-8 rounded-md bg-[#f3f6fb] p-5">
            <h2 class="font-black text-[#071a80]">Score</h2>
        ...
        @if ($match->status === 'pending_confirmation' && $match->players->contains('user_id', auth()->id()))
            <div class="mt-8 flex flex-wrap gap-3">
                <button wire:click="confirm" class="rounded-full bg-[#071a80] px-6 py-3 text-sm font-black uppercase text-white">Confirm result</button>
        ...
        @else
            <p class="mt-8 font-bold text-blue-950/60">Status: {{ str_replace('_', ' ', $match->status) }}</p>
        @endif
    </div>
</div>
