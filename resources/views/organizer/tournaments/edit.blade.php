    ...
    <div class="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <section class="rounded-lg bg-white p-6 shadow-lg">
            ...
            <form method="POST" action="{{ route('organizer.tournaments.categories.store', $tournament) }}" class="mt-5 grid gap-3 md:grid-cols-6">@csrf
                ...
                <button class="rounded-md bg-[#071a80] px-4 py-2 text-sm font-black uppercase text-white md:col-span-6">Add category</button>
            </form>
            <div class="mt-6 grid gap-3 md:grid-cols-2">
                @foreach ($tournament->categories as $category)
                    <div class="rounded-md border border-blue-950/10 p-4">
                        <h3 class="font-black text-[#071a80]">{{ $category->name }}</h3>
                        <p class="text-sm text-blue-950/60">{{ $category->format }} | {{ str_replace('_', ' ', $category->draw_mode) }} | {{ $category->status }}</p>
                    </div>
                @endforeach
            </div>
        </section>
