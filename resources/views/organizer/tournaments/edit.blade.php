    ...
    <div class="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <section id="categories" class="rounded-lg bg-white p-6 shadow-lg">
            ...
            <form method="POST" action="{{ route('organizer.tournaments.categories.store', $tournament) }}" class="mt-5 grid gap-3 md:grid-cols-7">@csrf
                ...
                <button class="rounded-md bg-brand-blue px-4 py-2 text-sm font-black uppercase text-white md:col-span-7">Add category</button>
            </form>
            <div class="mt-6 grid gap-3 md:grid-cols-2">
                @foreach ($tournament->categories as $category)
                    <div class="rounded-md border border-brand-ink/10 p-4">
                        <h3 class="font-black text-brand-blue">{{ $category->name }}</h3>
                        <p class="text-sm text-brand-ink/60">{{ $category->format }} | {{ str_replace('_', ' ', $category->draw_mode) }}{{ $category->draw_mode === 'round_robin' ? ' | groups of '.$category->group_size : '' }} | {{ $category->status }}</p>
                    </div>
                @endforeach
            </div>
        </section>
