        ...
        <form method="POST" action="{{ route('organizer.tournaments.draw-engine.preview', $tournament) }}" class="grid gap-6">
            ...
            <section class="rounded-lg bg-white p-6 shadow-lg">
                ...
                <div class="mt-4 grid gap-4 md:grid-cols-4">
                    ...
                    <label class="text-sm font-black text-brand-blue">Event priority
                        <input name="event_priority" type="number" min="1" value="{{ old('event_priority', 1) }}" class="mt-1 w-full rounded-md border-brand-ink/10 text-sm font-bold">
                    </label>
                    <label class="text-sm font-black text-brand-blue">Stage priority
                        <input name="stage_priority" type="number" min="1" value="{{ old('stage_priority', 1) }}" class="mt-1 w-full rounded-md border-brand-ink/10 text-sm font-bold">
                    </label>
                </div>
            </section>
            ...
            <section class="rounded-lg bg-white p-6 shadow-lg">
                ...
                <div class="mt-4 grid gap-4 md:grid-cols-6">
                    ...
                    <label class="text-sm font-black text-brand-blue">Court label
                        <input name="court_label_prefix" value="{{ old('court_label_prefix', 'Court') }}" class="mt-1 w-full rounded-md border-brand-ink/10 text-sm font-bold">
                    </label>
                    <label class="text-sm font-black text-brand-blue">Start
                        <input name="schedule_start_time" type="time" value="{{ old('schedule_start_time', '09:00') }}" class="mt-1 w-full rounded-md border-brand-ink/10 text-sm font-bold">
                    </label>
                    <label class="text-sm font-black text-brand-blue">End
                        <input name="schedule_end_time" type="time" value="{{ old('schedule_end_time', '18:00') }}" class="mt-1 w-full rounded-md border-brand-ink/10 text-sm font-bold">
                    </label>
                    <label class="text-sm font-black text-brand-blue">Duration
                        <input name="match_duration_minutes" type="number" min="5" max="240" value="{{ old('match_duration_minutes', 30) }}" class="mt-1 w-full rounded-md border-brand-ink/10 text-sm font-bold">
                    </label>
                    <label class="text-sm font-black text-brand-blue">Rest
                        <input name="rest_minutes" type="number" min="0" max="120" value="{{ old('rest_minutes', 10) }}" class="mt-1 w-full rounded-md border-brand-ink/10 text-sm font-bold">
                    </label>
                </div>
                    ...
                    @foreach ([1 => 'Pool matches only', 2 => 'Knockout round 1 / QF', 3 => 'Semifinal / Final'] as $index => $label)
                        <div class="rounded-md border border-brand-ink/10 p-4">
                            ...
                            <div class="mt-3 grid grid-cols-2 gap-2">
                                <input type="time" name="days[{{ $index - 1 }}][start_time]" value="{{ old("days.".($index - 1).".start_time", '09:00') }}" class="rounded-md border-brand-ink/10 text-sm font-bold">
                                <input type="time" name="days[{{ $index - 1 }}][end_time]" value="{{ old("days.".($index - 1).".end_time", '18:00') }}" class="rounded-md border-brand-ink/10 text-sm font-bold">
                            </div>
                            <input type="number" name="days[{{ $index - 1 }}][courts_count]" min="1" value="{{ old("days.".($index - 1).".courts_count", 2) }}" class="mt-3 w-full rounded-md border-brand-ink/10 text-sm font-bold">
                            <div class="mt-3 grid gap-2 text-xs font-bold text-brand-ink/70">
            ...
            <div class="flex flex-wrap gap-3">
                <button class="rounded-md bg-brand-blue px-5 py-3 text-xs font-black uppercase text-white">Preview generated draw</button>
            </div>
        </form>
            ...
            <section class="mt-8 rounded-lg bg-white p-6 shadow-lg">
                ...
                <div class="mt-5 grid gap-3 md:grid-cols-2">
                    @foreach ($preview['matches'] as $match)
                        <article class="rounded-md border border-brand-ink/10 p-4">
                            ...
                            <p class="mt-1 font-black text-brand-blue">{{ $match['side_b']?->displayName() ?? ($match['is_bye'] ? 'BYE' : 'TBA') }}</p>
                            <p class="mt-2 text-xs font-bold text-brand-ink/50">{{ $match['court_label'] ?? 'Unscheduled' }} {{ $match['scheduled_at'] ? '| '.$match['scheduled_at']->format('M j, g:i A') : '' }}</p>
                        </article>
                    @endforeach
                </div>
                ...
                <form method="POST" action="{{ route('organizer.tournaments.draw-engine.generate', $tournament) }}" class="mt-6 rounded-md bg-brand-surface p-4">
                    ...
                    @foreach ((array) request('days', []) as $dayIndex => $day)
                        ...
                        @endforeach
                    @endforeach
                    <label class="flex items-center gap-2 text-sm font-bold text-brand-ink/70">
