...
<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h1 class="text-3xl font-black text-brand-blue">Tournament schedule</h1>
        </div>
    </x-slot>
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <form class="mb-6 flex flex-col gap-3 rounded-lg bg-white p-5 shadow-lg sm:flex-row">
            <input name="date" type="date" value="{{ request('date') }}" class="rounded-md border-brand-ink/10">
            <button class="rounded-md bg-brand-blue px-4 py-2 text-sm font-black uppercase text-white">Filter date</button>
            <a href="{{ route('tournaments.matches', $tournament) }}" class="rounded-md border border-brand-ink/10 px-4 py-2 text-center text-sm font-black uppercase text-brand-blue">Clear</a>
        </form>
        ...
        <div class="grid gap-6">
            ...
            @forelse ($matches as $date => $dayMatches)
                <section class="rounded-lg bg-white p-6 shadow-lg">
                    <h2 class="text-2xl font-black text-brand-blue">{{ \Illuminate\Support\Carbon::parse($date)->format('M j, Y') }}</h2>
                    <div class="mt-5 grid gap-4">
            ...
            @empty
                @if ($liveMatches->isEmpty())
                    <section class="rounded-lg bg-white p-6 shadow-lg">
                        <h2 class="text-xl font-black text-brand-blue">No matches scheduled</h2>
                        <p class="mt-2 text-brand-ink/60">Generated draw matches will appear here.</p>
                    </section>
                @endif
            @endforelse
        </div>
